...
public class Sistema {
	...
	// TODO: O que fazer se a quantidade de frames for fracionária?

	// ------------------- G M - Implementa Paginação ----------------------
	public class GM {
		...
		int numFrames;
		boolean[] memFis;
		boolean[] tabela_paginas;

		public GM(int tamMem, int tamPag) {
			this.tamMem = tamMem;
			this.tamPag = tamPag;
			this.numFrames = tamMem / tamPag;
			this.memFis = new boolean[tamMem];
			this.tabela_paginas = new boolean[numFrames];
		}

		public boolean alocar(int num) {
			int qtd_pag_ocupadas = (int) Math.ceil(num / (double) tamPag);
			int[] pag_ocupadas = new int[numFrames];
			int cont_ocupacao = 0;
			
			for (int i = 0; i < tabela_paginas.length; i ++){
				if(tabela_paginas[i] == false){
					// Lista quais páginas foram ocupadas, em ordem.
					pag_ocupadas[cont_ocupacao] = i;
					cont_ocupacao++;
	
					if (cont_ocupacao == qtd_pag_ocupadas){
						// Ocupa as páginas
						for(int indice : pag_ocupadas){
							tabela_paginas[indice] = true;
							// Endereço Físico
							for(int j = indice * tamPag; j < (indice + 1) * tamPag; j++){
								memFis[j] = true;
								num--;
								if (num == 0){
									break;
						...
						imprimirTabelaPaginas(tabela_paginas);
		...
		public void desalocar(int[] lista) {
			for(int indice : lista){
				tabela_paginas[indice] = false;

				// O + 1 é para pegarmos toda a página.
				for(int i = indice * tamPag; i < (indice + 1) * tamPag; i ++){
					memFis[i] = false;
				}
			}
		}

		public void imprimirTabelaPaginas(boolean[] paginas) {

		String formato = "%-10s%-15s%n";
		System.out.println("\n\n-------------------------------------------------");
		System.out.printf(formato, "Página", "Ocupada");
		System.out.println("-------------------------------------------------");

		for (int i = 0; i < paginas.length; i++) {
			System.out.printf(formato, i, paginas[i]);
		}
	}

		public void imprimirMemoriaFisica(int inicio, int fim) {

			String formato = "%-10s%-15s%n";
			System.out.println("\n\n-------------------------------------------------");
			System.out.printf(formato, "Posição", "Ocupado");
			System.out.println("-------------------------------------------------");

			for (int i = inicio; i < fim; i++) {
				System.out.printf(formato, i, memFis[i]);
				if((i+1) % tamPag == 0){
					System.out.println("-------------------------------------------------");
				}
			}
	...
	public Sistema(int tamMem, int tamPag) { // a VM com tratamento de interrupções
	...
	public static void main(String args[]) {
		...
		// System.out.println("Informe o tamanho da memória.");
		// int tamMem = teclado.nextInt();
		// System.out.println("Informe o tamanho da página.");
		// int tamPag = teclado.nextInt();

		int tamMem = 32;
		Sistema s = new Sistema(tamMem, 8);


		s.gm.imprimirMemoriaFisica(0, tamMem);
		s.gm.imprimirTabelaPaginas(s.gm.tabela_paginas);
		teclado.nextLine();
